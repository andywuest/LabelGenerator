...
public class PDFGenerator {
	...
	public boolean generate(Layout layout, List<ButtonLabel> buttonLabelList,
			boolean drawGrid) {
		...
		if (FontUtil.isFontSupported(layout.getFontName()) == false) {
			logger
					.log(
							...
							new Object[]{layout.getFontName(),
									FontUtil.getSupportedFonts().toString()});
			return result;
		}

		try {
			logger.log(Level.INFO, "Creating tmp PDF file in directory {0}",
			...
			final BaseFont font = BaseFont.createFont(layout.getFontName(),
					"UTF-8", false);

			int index = 0;
			for (int row = 0; row < layout.getRows(); row++) {
				for (int col = layout.getColumns() - 1; col >= 0; col--) {
					final ButtonLabel buttonLabel = buttonLabelList
							.get(buttonLabelList.size() - index - 1);
					String text = buttonLabel.getRenderedTemplate();

					logger.log(Level.SEVERE, "Text {0} : {1} ", new Object[]{
							Integer.valueOf(index),
							buttonLabel.getRenderedTemplate()});

					final int x = mmToPoints(layout.getMarginLeft() + col
							* layout.getWidthNext());
					final int y = mmToPoints(layout.getMarginTop() + row
							* layout.getHeightNext());
					...
					if (drawGrid == true) {
						cb.rectangle(r.getLeft(), r.getTop(), r.getWidth(), r
								.getHeight());
						cb.stroke();
					...
					if (lines != null && lines.length > 0) {
						for (String tmpLine : lines) {
							...
							cb.showTextAligned(PdfContentByte.ALIGN_LEFT,
									StringUtils.trim(tmpLine), x
											+ mmToPoints(layout
													.getTextMarginLeft()
													.floatValue()), y
											+ height
											- (offset * (layout.getFontSize()
													.floatValue() + layout
													.getTextMargin()
													.floatValue()))
											- mmToPoints(layout
	...
	private Rectangle getRectangle(int x, int y, int width, int height) {
		Rectangle r = new Rectangle(x, PageSize.A4.getHeight() - y + height, x
				+ width, PageSize.A4.getHeight() - y);
		return r;
	}
